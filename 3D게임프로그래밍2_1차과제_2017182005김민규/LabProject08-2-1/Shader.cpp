...
void CBillboardShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nBillboardObjects, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CBillboardShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	CreateCbvSrvDescriptorHeaps(pd3dDevice, m_nBillboardObjects, 1);
	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	CreateConstantBufferViews(pd3dDevice, m_nBillboardObjects, m_pd3dcbGameObjects, ncbElementBytes);
...
void CBillboardShader::ReleaseObjects()
{
	...
	{
		for (int j = 0; j < m_nBillboardObjects; j++) if (m_ppBillboardObjects[j]) delete m_ppBillboardObjects[j];
...
void CBillboardShader::ReleaseUploadBuffers()
{
	if (m_ppBillboardObjects)
	{
		for (int j = 0; j < m_nBillboardObjects; j++) if (m_ppBillboardObjects[j]) m_ppBillboardObjects[j]->ReleaseUploadBuffers();
	}
...
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbBulletObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nBulletObjects, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);

	m_pd3dcbBulletObjects->Map(0, NULL, (void **)&m_pcbMappedBulletObjects);
}

void CBulletShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255);
	for (int j = 0; j < m_nBulletObjects; j++)
	{
		CB_GAMEOBJECT_INFO *pbMappedcbGameObject = (CB_GAMEOBJECT_INFO *)((UINT8 *)m_pcbMappedBulletObjects + (j * ncbElementBytes));
		XMStoreFloat4x4(&pbMappedcbGameObject->m_xmf4x4World, XMMatrixTranspose(XMLoadFloat4x4(&m_ppBulletObjects[j]->m_xmf4x4World)));
	}
}

void CBulletShader::ReleaseShaderVariables()
{
	if (m_pd3dcbBulletObjects)
	{
		m_pd3dcbBulletObjects->Unmap(0, NULL);
		m_pd3dcbBulletObjects->Release();
	}
...
void CBulletShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	CreateCbvSrvDescriptorHeaps(pd3dDevice, m_nBulletObjects, 1);
	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	CreateConstantBufferViews(pd3dDevice, m_nBulletObjects, m_pd3dcbBulletObjects, ncbElementBytes);
	CreateShaderResourceViews(pd3dDevice, pTexture, 0, 3);
	...
	CCubeMeshTextured* pBulletMesh = new CCubeMeshTextured(pd3dDevice, pd3dCommandList, 5.0f, 5.0f, 2.5f);

	m_ppBulletObjects = new CBulletObject * [m_nBulletObjects];
	CBulletObject* pBulletObject = NULL;
	for (int i = 0; i < m_nBulletObjects; i++)
	{
		...
		m_ppBulletObjects[i] = pBulletObject;
	}
}

void CBulletShader::FireBullet()
{
	...
	for (int i = 0; i < m_nBulletObjects; i++)
	{
		if (!m_ppBulletObjects[i]->m_bActive)
		{
			pBulletObject = m_ppBulletObjects[i];
			break;
		}
	}
	if (pBulletObject)
	{
		...
		pBulletObject->m_xmf4x4World = m_pPlayer->m_xmf4x4World;
...
{
	if (m_ppBulletObjects)
	{
		for (int j = 0; j < m_nBulletObjects; j++) if (m_ppBulletObjects[j]) delete m_ppBulletObjects[j];
		delete[] m_ppBulletObjects;
	}
...
void CBulletShader::AnimateObjects(float fTimeElapsed, CCamera* pCamera)
{
	for (int j = 0; j < m_nBulletObjects; j++)
	{
		if(m_ppBulletObjects[j]->m_bActive)
			m_ppBulletObjects[j]->Animate(fTimeElapsed, pCamera);
	}
}
...
void CBulletShader::ReleaseUploadBuffers()
{
	if (m_ppBulletObjects)
	{
		for (int j = 0; j < m_nBulletObjects; j++)
		{
				if (m_ppBulletObjects[j]) 
			    	m_ppBulletObjects[j]->ReleaseUploadBuffers();
		}
	}
...
void CTerrainWaterShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState*[m_nPipelineStates];
