...
void CTerrainPlayer::CheckPlayerByWallCollision()
{
	CHeightMapTerrain* pTerrain = (CHeightMapTerrain*)m_pPlayerUpdatedContext;
	float fWidth = pTerrain->GetWidth(), fLength = pTerrain->GetLength();
	...
	if (x > fWidth - 2.0f)
	{
		SetPos = XMFLOAT3(fWidth - 2.0f, y, z);
		SetPosition(SetPos);
	}
	else if (x < +2.0f)
	{
		SetPos = XMFLOAT3(2.0f, y, z);
		SetPosition(SetPos);
	}
	if (z > fLength - 2.0f)
	{
		SetPos = XMFLOAT3(x, y, fLength - 2.0f);
		SetPosition(SetPos);
	}
	else if (z < 2.0f)
	{
		SetPos = XMFLOAT3(x, y, 2.0f);
		SetPosition(SetPos);
	}
}

void CTerrainPlayer::Animate(float fElapsedTime, CCamera *pCamera)
{
	CheckPlayerByWallCollision();
	CPlayer::Animate(fElapsedTime, pCamera);
}
